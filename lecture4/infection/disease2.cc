...
public: 
  ...
  void random_infection(){
    ...
    the_population[infected].infect(5);
  }
  int count_infected(){
    int num_infected = 0;
  ...
  void contacts(int intended_contacts,float probability){   
	...
	if (probability > chance_of_infection and the_population[i].get_status()==0)
	  {the_population[random].infect(i+5);}
      }
      people_contacted++;
    }
  };
...
int main (){
  ...
  cout << "Now let's account for 6 random interactions among the populations " << endl; 
  Population spread(popsize);
  spread.set_vaccinated(vac_percent);
  spread.random_infection();
  spread.contacts(6,probability);
  int days4 = 0;
